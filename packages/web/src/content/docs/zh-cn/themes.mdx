...
通过 FangCode，您可以从多个内置主题中进行选择，使用能自动适配终端主题的主题，或者定义您自己的自定义主题。

默认情况下，FangCode 使用我们自己的 `fangcode` 主题。
...
FangCode 支持灵活的基于 JSON 的主题系统，让用户可以轻松创建和自定义主题。
...
1. **内置主题** — 嵌入在二进制文件中
2. **用户配置目录** — 定义在 `~/.config/fangcode/themes/*.json` 或 `$XDG_CONFIG_HOME/fangcode/themes/*.json`
