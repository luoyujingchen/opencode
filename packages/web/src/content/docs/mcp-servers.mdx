...
You can add external tools to FangCode using the _Model Context Protocol_, or MCP. FangCode supports both local and remote servers.
...
You can define MCP servers in your [FangCode Config](https://fangcode.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
FangCode automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, FangCode will:
...
If the server requires authentication, FangCode will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `fangcode mcp auth <server-name>`.
...
```bash
fangcode mcp auth my-oauth-server
```
...
```bash
fangcode mcp logout my-oauth-server
```

The `mcp auth` command will open your browser for authorization. After you authorize, FangCode will store the tokens securely in `~/.local/share/fangcode/mcp-auth.json`.
...
fangcode mcp debug my-oauth-server
```
...
Your MCPs are available as tools in FangCode, alongside built-in tools. So you can manage them through the FangCode config like any other tool.
...
```bash
fangcode mcp auth sentry
```

This will open a browser window to complete the OAuth flow and connect FangCode to your Sentry account.
