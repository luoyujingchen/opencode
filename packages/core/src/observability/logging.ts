...
export function minimumLogLevel() {
  const value = (process.env.FANG_LOG_LEVEL ?? process.env.OPENCODE_LOG_LEVEL)?.toUpperCase()
  const levels = {
    DEBUG: "Debug",
    INFO: "Info",
    WARN: "Warn",
...
export function loggers() {
  return process.env.FANG_PRINT_LOGS === "1" || process.env.OPENCODE_PRINT_LOGS === "1"
