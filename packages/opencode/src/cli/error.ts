...
export function FormatError(input: unknown) {
  if (MCP.Failed.isInstance(input))
    return `MCP server "${input.data.name}" failed. Note, fangcode does not support MCP authentication yet.`
  ...
  if (Provider.ModelNotFoundError.isInstance(input)) {
    ...
    return [
      ...
      `Try: \`fangcode models\` to list available models`,
      `Or check your config (fangcode.json or opencode.json) provider/model names`,
    ].join("\n")
  }
