...
Custom tools are functions you create that the LLM can call during conversations. They work alongside fangcode's [built-in tools](/docs/tools) like `read`, `write`, and `bash`.
...
- Or globally, by placing them in `~/.config/fangcode/tools/`.
...
import { tool } from "@fangcode/plugin"
