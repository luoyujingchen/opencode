---
title: Plugins
description: Write your own plugins to extend FangCode.
---

Plugins allow you to extend FangCode by hooking into various events and customizing behavior. You can create plugins to add new features, integrate with external services, or modify FangCode's default behavior.
...
- `~/.config/fangcode/plugins/` - Global plugins
...
**npm plugins** are installed automatically using Bun at startup. Packages and their dependencies are cached in `~/.cache/fangcode/node_modules/`.
...
1. Global config (`~/.config/fangcode/opencode.json`)
2. Project config (`opencode.json`)
3. Global plugin directory (`~/.config/fangcode/plugins/`)
...
FangCode runs `bun install` at startup to install these. Your plugins and tools can then import them.
...
- `client`: An fangcode SDK client for interacting with the AI.
...
import type { Plugin } from "@fangcode/plugin"
  ...
  return {
    event: async ({ event }) => {
      ...
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "fangcode"'`
      }
    },
  }
}
...
:::note
If you’re using the FangCode desktop app, it can send system notifications automatically when a response is ready or when a session errors.
:::
...
Prevent fangcode from reading `.env` files:
...
Plugins can also add custom tools to fangcode:

```ts title=".opencode/plugins/custom-tools.ts"
import { type Plugin, tool } from "@fangcode/plugin"
...
The `tool` helper creates a custom tool that fangcode can call. It takes a Zod schema function and returns a tool definition with:

- `description`: What the tool does
- `args`: Zod schema for the tool's arguments
- `execute`: Function that runs when the tool is called

Your custom tools will be available to fangcode alongside built-in tools.
...
Levels: `debug`, `info`, `warn`, `error`. See [SDK documentation](https://fangcode.ai/docs/sdk) for details.
