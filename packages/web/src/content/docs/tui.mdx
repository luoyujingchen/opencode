---
title: TUI
description: Using the FangCode terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

FangCode provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running FangCode starts the TUI for the current directory.
...
When using the FangCode TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to FangCode. Allows you to select from available providers and add their API keys.
...
Exit FangCode. _Aliases_: `/quit`, `/q`
...
- `leader_timeout` - Controls how long FangCode waits after the leader key. Defaults to `2000`.
