---
title: CLI
description: FangCode CLI options and commands.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

The FangCode CLI by default starts the [TUI](/docs/tui) when run without any arguments.

```bash
fangcode
```

But it also accepts commands as documented on this page. This allows you to interact with FangCode programmatically.

```bash
fangcode run "Explain how closures work in JavaScript"
```
...
Start the FangCode terminal user interface.
...
The FangCode CLI also has the following commands.

---

### agent

Manage agents for FangCode.

```bash
fangcode agent [command]
```
...
Attach a terminal to an already running FangCode backend server started via `serve` or `web` commands.

```bash
fangcode attach [url]
```

This allows using the TUI with a remote FangCode backend. For example:

```bash
# Start the backend server for web/mobile access
fangcode web --port 4096 --hostname 0.0.0.0

# In another terminal, attach the TUI to the running backend
fangcode attach http://10.20.30.40:4096
```
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Basic auth username (defaults to `OPENCODE_SERVER_USERNAME` or `fangcode`) |
...
FangCode is powered by the provider list at [Models.dev](https://models.dev), so you can use `fangcode auth login` to configure API keys for any provider you'd like to use. This is stored in `~/.local/share/fangcode/auth.json`.

```bash
fangcode auth login
```

When FangCode starts up it loads the providers from the credentials file. And if there are any keys defined in your environments or a `.env` file in your project.
...
```bash
fangcode mcp auth list
```

Or use the short version.

```bash
fangcode mcp auth ls
```
...
Use the `--refresh` flag to update the cached model list. This is useful when new models have been added to a provider and you want to see them in FangCode.

```bash
fangcode models --refresh
```
...
Run fangcode in non-interactive mode by passing a prompt directly.

```bash
fangcode run [message..]
```

This is useful for scripting, automation, or when you want a quick answer without launching the full TUI. For example.

```bash "fangcode run"
fangcode run Explain the use of context in Go
```

You can also attach to a running `fangcode serve` instance to avoid MCP server cold boot times on every run:
...
fangcode run --attach http://localhost:4096 "Explain async/await in JavaScript"
```
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Attach to a running fangcode server (e.g., http://localhost:4096)          |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Basic auth password (defaults to `OPENCODE_SERVER_PASSWORD`)               |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Basic auth username (defaults to `OPENCODE_SERVER_USERNAME` or `fangcode`) |
...
Start a headless FangCode server for API access. Check out the [server docs](/docs/server) for the full HTTP interface.

```bash
fangcode serve
```

This starts an HTTP server that provides API access to fangcode functionality without the TUI interface. Set `OPENCODE_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `fangcode`).
...
Manage FangCode sessions.

```bash
fangcode session [command]
```
...
List all FangCode sessions.

```bash
fangcode session list
```
...
Delete an FangCode session.

```bash
fangcode session delete <sessionID>
```
...
Show token usage and cost statistics for your FangCode sessions.
...
Import session data from a JSON file or FangCode share URL.

```bash
fangcode import <file>
```

You can import from a local file or an FangCode share URL.

```bash
fangcode import session.json
fangcode import https://opncd.ai/s/abc123
```
...
Start a headless FangCode server with a web interface.

```bash
fangcode web
```

This starts an HTTP server and opens a web browser to access FangCode through a web interface. Set `OPENCODE_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `fangcode`).
...
```bash
fangcode plugin <module>
```

Or use the alias.

```bash
fangcode plug <module>
```
...
Fetch and checkout a GitHub PR branch, then run FangCode.
...
Uninstall FangCode and remove all related files.
...
Updates fangcode to the latest version or a specific version.

```bash
fangcode upgrade [target]
```
...
The fangcode CLI takes the following global flags.
...
FangCode can be configured using environment variables.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Override basic auth username (default `fangcode`) |
