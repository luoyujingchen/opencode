...
With FangCode you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, FangCode uses our own `fangcode` theme.
...
FangCode comes with several built-in themes.
...
- Want FangCode to match their terminal's appearance
...
FangCode supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/fangcode/themes/*.json` or `$XDG_CONFIG_HOME/fangcode/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/fangcode/themes
vim ~/.config/fangcode/themes/my-theme.json
```
...
{
  "$schema": "https://fangcode.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
