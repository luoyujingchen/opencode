  ...
  async handler(args) {
    await Instance.provide({
      ...
      async fn() {
          ...
          if (prInfoText.trim()) {
            ...
            if (prInfo && prInfo.body) {
              ...
              if (sessionMatch) {
                ...
                const importResult = await Process.text(["fangcode", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        const opencodeProcess = Process.spawn(["fangcode", ...opencodeArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
        ...
        if (code !== 0) throw new Error(`fangcode exited with code ${code}`)
      },
    })
  },
})
