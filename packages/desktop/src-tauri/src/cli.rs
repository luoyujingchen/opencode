...
const CLI_INSTALL_DIR: &str = ".fangcode/bin";
const CLI_BINARY_NAME: &str = "fangcode";
...
pub fn spawn_command(
    ...
    let mut cmd = if cfg!(windows) {
        if is_wsl_enabled(app) {
            ...
            let mut script = vec![
                "set -e".to_string(),
                "BIN=\"$HOME/.fangcode/bin/fangcode\"".to_string(),
...
pub fn serve(
    ...
    let envs = [
        ("OPENCODE_SERVER_USERNAME", "fangcode".to_string()),
