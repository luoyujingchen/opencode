...
// etc.).
//
// The visual language mirrors the FangCode app: the design tokens are a curated
...
export function success(options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider ? `FangCode is now connected to ${escapeHtml(provider)}.` : "FangCode is now authorized.",
...
export function error(detail: string, options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider
        ? `FangCode couldn't finish connecting to ${escapeHtml(provider)}.`
        : "FangCode couldn't complete authorization.",
      detail,
      footnote: "Close this window and try again from FangCode.",
    }),
  })
}
...
<html lang="en">
  <head>
    ...
    <title>${escapeHtml(input.title)} · FangCode</title>
    <style>${STYLES}</style>
  </head>
  <body>
...
(function(){
  ...
  function fail(text){card.dataset.status="error";headline.textContent="Authorization failed";message.textContent=PROVIDER?("FangCode couldn't finish connecting to "+PROVIDER+"."):"FangCode couldn't complete authorization.";if(text){detail.textContent=text;detail.hidden=false}footnote.textContent="Close this window and try again from FangCode."}
  function ok(){card.dataset.status="success";headline.textContent="Authorization successful";message.textContent=PROVIDER?("FangCode is now connected to "+PROVIDER+"."):"FangCode is now authorized.";detail.hidden=true;footnote.textContent="You can close this window.";setTimeout(function(){try{window.close()}catch(e){}},2500)}
  try{
...
const STYLES = `
  ...
  * { box-sizing: border-box; }
  ...
  .brand-wordmark { font-size: 1.125rem; font-weight: 650; letter-spacing: 0; color: var(--oc-text-strong); }
...
const WORDMARK = `<span class="brand-wordmark" role="img" aria-label="FangCode">FangCode</span>`
