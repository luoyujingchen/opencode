...
export async function installWslOpencode(version: string, distro: string, opts?: RunWslOptions) {
  const url = process.env.FANG_INSTALL_URL ?? process.env.OPENCODE_INSTALL_URL ?? "https://fangcode.ai/install"
  return runInteractiveCommand(
    resolveSystem32Command("wsl.exe"),
    wslArgs(["bash", "-lc", `curl -fsSL ${shellEscape(url)} | bash -s -- --version ${shellEscape(version)}`], distro),
    withTimeout(opts, DEFAULT_WSL_INSTALL_TIMEOUT_MS),
    DEFAULT_WSL_INSTALL_TIMEOUT_MS,
  )
}
...
export async function resolveWslOpencode(distro: string, opts?: RunWslOptions) {
  return firstLine(
    (
      await runWslSh(
        'if [ -x "$HOME/.fangcode/bin/fangcode" ]; then printf "%s\\n" "$HOME/.fangcode/bin/fangcode"; elif [ -x "$HOME/.opencode/bin/opencode" ]; then printf "%s\\n" "$HOME/.opencode/bin/opencode"; fi',
        distro,
        opts,
      )
    ).stdout,
