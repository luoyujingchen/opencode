...
To debug issues with FangCode, start by checking the logs and local data it stores on disk.
...
- **macOS/Linux**: `~/.local/share/fangcode/log/`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.local\share\fangcode\log`

Log files are named with timestamps (e.g., `2025-01-09T123456.log`) and the most recent 10 log files are kept.

You can set the log level with the `--log-level` command-line option to get more detailed debug information. For example, `fangcode --log-level DEBUG`.

---

## Storage

fangcode stores session data and other application data on disk at:

- **macOS/Linux**: `~/.local/share/fangcode/`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.local\share\fangcode`
...
To uninstall the FangCode CLI and remove its related files, run:
...
To remove FangCode Desktop, uninstall the application through your operating system's app management tools.

---

## Desktop app

FangCode Desktop runs a local FangCode server (the `opencode-cli` sidecar) in the background. Most issues are caused by a misbehaving plugin, a corrupted cache, or a bad server setting.

### Quick checks

- Fully quit and relaunch the app.
- If the app shows an error screen, click **Restart** and copy the error details.
- macOS only: `FangCode` menu -> **Reload Webview** (helps if the UI is blank/frozen).
...
- **macOS/Linux**: `~/.config/fangcode/opencode.jsonc` (or `~/.config/fangcode/opencode.json`)
- **macOS/Linux** (older installs): `~/.local/share/opencode/opencode.jsonc`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.config\fangcode\opencode.jsonc`
...
FangCode can also load local plugins from disk. Temporarily move these out of the way (or rename the folder) and restart the desktop app:

- **Global plugins**
  - **macOS/Linux**: `~/.config/fangcode/plugins/`
  - **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.config\fangcode\plugins`
...
If disabling plugins doesn't help (or a plugin install is stuck), clear the cache so FangCode can rebuild it.

1. Quit FangCode Desktop completely.
2. Delete the cache directory:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/fangcode`
- **Linux**: delete `~/.cache/fangcode` (or run `rm -rf ~/.cache/fangcode`)
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.cache\fangcode`

3. Restart FangCode Desktop.

---

### Fix server connection issues

FangCode Desktop can either start its own local server (default) or connect to a server URL you configured.
...
On Windows, FangCode Desktop requires the Microsoft Edge **WebView2 Runtime**. If the app opens to a blank window or won't start, install/update WebView2 and try again.

---

### Windows: General performance issues

If you're experiencing slow performance, file access issues, or terminal problems on Windows, try using [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL provides a Linux environment that works more seamlessly with FangCode's features.

---

### Notifications not showing

FangCode Desktop only shows system notifications when:

- notifications are enabled for FangCode in your OS settings, and
...
1. Quit FangCode Desktop.
2. Find and delete these files (they live in the FangCode Desktop app data directory):
...
If you're experiencing issues with FangCode:

1. **Report issues on GitHub**

   The best way to report bugs or request features is through our GitHub repository:

   [**github.com/anomalyco/fangcode/issues**](https://github.com/anomalyco/fangcode/issues)
...
2. **Join our Discord**
   ...
   [**fangcode.ai/discord**](https://fangcode.ai/discord)
...
### FangCode won't start

1. Check the logs for error messages
2. Try running with `--print-logs` to see output in the terminal
3. Ensure you have the latest version with `fangcode upgrade`
...
To figure out what models you have access to, run `fangcode models`
...
2. If the issue persists, try clearing your stored configuration:
   ...
   ```bash
   rm -rf ~/.local/share/fangcode
   ```

   On Windows, press `WIN+R` and delete: `%USERPROFILE%\.local\share\fangcode`
...
If you encounter API call errors, this may be due to outdated provider packages. fangcode dynamically installs provider packages (OpenAI, Anthropic, Google, etc.) as needed and caches them locally.
...
1. Clear the provider package cache:
   ...
   ```bash
   rm -rf ~/.cache/fangcode
   ```

   On Windows, press `WIN+R` and delete: `%USERPROFILE%\.cache\fangcode`

2. Restart fangcode to reinstall the latest provider packages

This will force fangcode to download the most recent versions of provider packages, which often resolves compatibility issues with model parameters and API changes.
...
fangcode will detect if you're using Wayland and prefer `wl-clipboard`, otherwise it will try to find clipboard tools in order of: `xclip` and `xsel`.
