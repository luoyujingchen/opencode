...
const releaseId = process.env.FANG_RELEASE ?? process.env.OPENCODE_RELEASE
if (!releaseId) throw new Error("FANG_RELEASE or OPENCODE_RELEASE is required")

const version = process.env.FANG_VERSION ?? process.env.OPENCODE_VERSION
if (!version) throw new Error("FANG_VERSION or OPENCODE_VERSION is required")
...
const macxTarGz = "fangcode-desktop-mac-x64.app.tar.gz"
const macaTarGz = "fangcode-desktop-mac-arm64.app.tar.gz"
