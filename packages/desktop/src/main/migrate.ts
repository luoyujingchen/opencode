...
// `opencode.settings.dat` is special: it maps to the `fangcode.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"fangcode.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // opencode.settings.dat → the electron settings store ("fangcode.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "fangcode.global.dat").
  const storeName = filename === "opencode.settings.dat" ? "fangcode.settings" : filename
