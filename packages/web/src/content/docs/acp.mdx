---
title: ACP Support
description: Use FangCode in any ACP-compatible editor.
---

FangCode supports the [Agent Client Protocol](https://agentclientprotocol.com) or (ACP), allowing you to use it directly in compatible editors and IDEs.
...
To use FangCode via ACP, configure your editor to run the `fangcode acp` command.

The command starts FangCode as an ACP-compatible subprocess that communicates with your editor over JSON-RPC via stdio.
...
```json title="acp.json"
{
  "agent_servers": {
    "FangCode": {
      "command": "/absolute/path/bin/fangcode",
      "args": ["acp"]
    }
  }
}
...
To open it, use the new 'FangCode' agent in the AI Chat agent selector.
...
To use FangCode as an ACP agent in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), add the following to your Neovim config:
...
This config sets up CodeCompanion to use FangCode as the ACP agent for chat.
...
FangCode works the same via ACP as it does in the terminal. All features are supported:
...
- MCP servers configured in your FangCode config
