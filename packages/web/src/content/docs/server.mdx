---
title: Server
description: Interact with fangcode server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `fangcode serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an fangcode client can use.
...
```bash
fangcode serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
```bash
fangcode serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Set `OPENCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `fangcode`, or set `OPENCODE_SERVER_USERNAME` to override it. This applies to both `fangcode serve` and `fangcode web`.

```bash
OPENCODE_SERVER_PASSWORD=your-password fangcode serve
```
...
When you run `fangcode` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the fangcode server to interact with fangcode programmatically.
:::

This architecture lets fangcode support multiple clients and allows you to interact with fangcode programmatically.

You can run `fangcode serve` to start a standalone server. If you have the
fangcode TUI running, `fangcode serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the FangCode [IDE](/docs/ide) plugins.
...
The fangcode server exposes the following APIs.
