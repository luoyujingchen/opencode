...
const { client, server } = createFangCode()
...
function createFangCode() {
  const host = "127.0.0.1"
  const port = 4096
  const url = `http://${host}:${port}`
  const proc = spawn(`fangcode`, [`serve`, `--hostname=${host}`, `--port=${port}`], {
    env: { ...process.env, FANG_INTERNAL_COMMANDS: "1", OPENCODE_INTERNAL_COMMANDS: "1" },
  })
...
function assertPayloadKeyword() {
  ...
  if (!body.match(/(?:^|\s)(?:\/fangcode|\/fang)(?=$|\s)/)) {
    throw new Error("Comments must mention `/fangcode` or `/fang`")
  }
}
...
async function assertFangCodeConnected() {
  let retry = 0
  let connected = false
  do {
    try {
  ...
  if (!connected) {
    throw new Error("Failed to connect to FangCode server")
  }
}
...
function useShareUrl() {
  return isMock() ? "https://dev.fangcode.ai" : "https://fangcode.ai"
}
...
async function getAccessToken() {
  ...
  let response
  if (isMock()) {
    response = await fetch("https://api.fangcode.ai/exchange_github_app_token_with_pat", {
      method: "POST",
      headers: {
    ...
    const oidcToken = await core.getIDToken("fangcode-github-action")
    response = await fetch("https://api.fangcode.ai/exchange_github_app_token", {
      method: "POST",
      headers: {
...
async function getUserPrompt() {
  ...
  let prompt = (() => {
    const body = payload.comment.body.trim()
    if (body === "/fangcode" || body === "/fang") {
      if (reviewContext) {
        return `Review this code change and suggest improvements for the commented lines:\n\nFile: ${reviewContext.file}\nLines: ${reviewContext.line}\n\n${reviewContext.diffHunk}`
      }
      return "Summarize this thread"
    }
    if (body.includes("/fangcode") || body.includes("/fang")) {
      if (reviewContext) {
...
function generateBranchName(type: "issue" | "pr") {
  const timestamp = new Date()
    ...
    .split("T")
    .join("")
  return `fangcode/${type}${useIssueId()}-${timestamp}`
}
...
function footer(opts?: { image?: boolean }) {
  ...
  const image = (() => {
    ...
    return `<a href="${useShareUrl()}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/fangcode-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
  })()
  const shareUrl = shareId ? `[FangCode session](${useShareUrl()}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
