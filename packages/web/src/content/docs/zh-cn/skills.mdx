...
代理技能让 FangCode 能够从你的仓库或主目录中发现可复用的指令。
...
- 全局配置：`~/.config/fangcode/skills/<name>/SKILL.md`
...
对于项目本地路径，FangCode 会从当前工作目录向上遍历，直到到达 git 工作树根目录。
在此过程中，它会加载 `.opencode/` 中所有匹配的 `skills/*/SKILL.md`，以及匹配的 `.claude/skills/*/SKILL.md` 或 `.agents/skills/*/SKILL.md`。

全局定义也会从 `~/.config/fangcode/skills/*/SKILL.md`、`~/.claude/skills/*/SKILL.md` 和 `~/.agents/skills/*/SKILL.md` 中加载。
